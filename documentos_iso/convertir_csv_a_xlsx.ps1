...
$csvFiles = Get-ChildItem -Path $baseDir -Filter "*.csv" | Sort-Object Name
...
function New-XlsxFromCsv($csvPath, $xlsxPath) {
    $rows = @(Import-Csv -Path $csvPath -Delimiter "," -Encoding UTF8)
    if ($rows.Count -eq 0) {
        Write-Host "Sin filas: $csvPath"
        return
    }

    $headers = @($rows[0].PSObject.Properties.Name)
    $tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("ss_xlsx_" + [guid]::NewGuid().ToString("N"))
    ...
    $sheetName = [System.IO.Path]::GetFileNameWithoutExtension($csvPath)
    ...
    for ($c = 0; $c -lt $headers.Count; $c++) {
        $max = [Math]::Min(60, [Math]::Max(10, ([string]$headers[$c]).Length + 2))
        foreach ($row in $rows) {
            $len = ([string]$row.($headers[$c])).Length + 2
            if ($len -gt $max) { $max = [Math]::Min(60, $len) }
        }
        $widths += $max
    }
    ...
    for ($c = 0; $c -lt $headers.Count; $c++) {
        $i = $c + 1
        [void]$cols.Append("<col min=""$i"" max=""$i"" width=""$($widths[$c])"" customWidth=""1""/>")
    }
...
"@
    ...
    Write-Host "Creado: $xlsxPath"
}

foreach ($file in $csvFiles) {
    $xlsxPath = Join-Path $baseDir ($file.BaseName + ".xlsx")
    New-XlsxFromCsv $file.FullName $xlsxPath
}
