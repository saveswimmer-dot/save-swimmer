...
function Label-Rotation([double]$avgRot, [double]$symmetry) {
    ...
    if ($avgRot -lt 35) { return "moderada" }
    if ($avgRot -le 55) { return "funcional" }
    if ($avgRot -le 65) { return "alta contextual" }
    return "muy alta"
}

function Rotation-Zone([double]$avgRot) {
    if ($avgRot -lt 20) { return "baja" }
    if ($avgRot -lt 35) { return "moderada" }
    if ($avgRot -le 55) { return "funcional_inicial" }
    if ($avgRot -le 65) { return "alta_contextual" }
    return "muy_alta_revisar"
}

function Side-ZoneStats($data, [string]$side) {
    $values = @()
    $zoneSec = 0.0
    $functionalSec = 0.0
    $competitiveSec = 0.0
    ...
    for ($i = 1; $i -lt $data.Count; $i++) {
        $prev = $data[$i - 1]
        $cur = $data[$i]
        $dt = [Math]::Max(0.0, [Math]::Min(1.5, $cur.t - $prev.t))
        $rot = [double]$cur.rot
        $isSide = if ($side -eq "R") { $rot -gt 0 } else { $rot -lt 0 }
        if (-not $isSide) {
            $currentStreak = 0.0
            continue
        }
        ...
        $values += $absRot
        $inFunctional = $absRot -ge 35 -and $absRot -le 55
        $inCompetitive = $absRot -gt 55 -and $absRot -le 65
        $inUsefulZone = $inFunctional -or $inCompetitive

        if ($inFunctional) { $functionalSec += $dt }
        if ($inCompetitive) { $competitiveSec += $dt }
        if ($inUsefulZone) {
            $zoneSec += $dt
            $usefulValues += $absRot
            $currentStreak += $dt
            if ($currentStreak -gt $maxStreak) { $maxStreak = $currentStreak }
        } else {
            $currentStreak = 0.0
        }
        if ($inFunctional) { $functionalValues += $absRot }
        if ($inCompetitive) { $competitiveValues += $absRot }
    }

    $avgGeneral = if ($values.Count -eq 0) { 0.0 } else { ($values | Measure-Object -Average).Average }
    $maxDeg = if ($values.Count -eq 0) { 0.0 } else { ($values | Measure-Object -Maximum).Maximum }
    $maxUsefulDeg = if ($usefulValues.Count -eq 0) { 0.0 } else { ($usefulValues | Measure-Object -Maximum).Maximum }
    $maxFunctionalDeg = if ($functionalValues.Count -eq 0) { 0.0 } else { ($functionalValues | Measure-Object -Maximum).Maximum }
    $maxCompetitiveDeg = if ($competitiveValues.Count -eq 0) { 0.0 } else { ($competitiveValues | Measure-Object -Maximum).Maximum }
    return [pscustomobject]@{
        generalAvg = $avgGeneral
        maxDeg = $maxDeg
        maxUsefulDeg = $maxUsefulDeg
        maxFunctionalDeg = $maxFunctionalDeg
        maxCompetitiveDeg = $maxCompetitiveDeg
        zoneSec = $zoneSec
        functionalSec = $functionalSec
        competitiveSec = $competitiveSec
        maxStreakSec = $maxStreak
    }
}
...
$idxRotDeg = ColIndex @("rot_deg", "rotation_deg")
$idxAlignDeg = ColIndex @("align_deg", "alignment_deg")
$idxBody = ColIndex @("body", "body_state")
...
for ($i = $headerLine + 1; $i -lt $lines.Count; $i++) {
    ...
    $rotDeg = if ($idxRotDeg -ge 0 -and $v.Count -gt $idxRotDeg) { Num $v[$idxRotDeg] } else { [double]::NaN }
    $alignDeg = if ($idxAlignDeg -ge 0 -and $v.Count -gt $idxAlignDeg) { Num $v[$idxAlignDeg] } else { [double]::NaN }
    $body = if ($idxBody -ge 0 -and $v.Count -gt $idxBody) { ([string]$v[$idxBody]).Trim().ToUpperInvariant() } else { "" }
    if ($body -eq "NO_SENSOR" -or $mag -lt 2.0 -or [Math]::Abs($lr) -gt 30 -or [Math]::Abs($fb) -gt 30 -or [Math]::Abs($ud) -gt 30) { continue }
    $local = if ($idxLocal -ge 0 -and $v.Count -gt $idxLocal) { $v[$idxLocal] } else { "" }
    $rows.Add([pscustomobject]@{
        t = $t; local = $local; lr = $lr; fb = $fb; ud = $ud; mag = $mag; pitch = $pitch; roll = $roll; rotDeg = $rotDeg; alignDeg = $alignDeg; body = $body; lat = $lat; lon = $lon; speed = $speed
    })
}
...
$zeroRotDeg = Median ($baseRows | Where-Object { ![double]::IsNaN($_.rotDeg) } | ForEach-Object { $_.rotDeg })
$zeroAlignDeg = Median ($baseRows | Where-Object { ![double]::IsNaN($_.alignDeg) } | ForEach-Object { $_.alignDeg })
...
foreach ($r in $rows) {
    ...
    $udD = $r.ud - $zeroUd
    $magD = $r.mag - $zeroMag
    if (![double]::IsNaN($r.rotDeg) -and ![double]::IsNaN($zeroRotDeg)) {
        $rot = $r.rotDeg - $zeroRotDeg
    } else {
        $rot = [Math]::Atan2($fbD, [Math]::Max(0.01, [Math]::Abs($r.ud))) * 180.0 / [Math]::PI
    }
    if (![double]::IsNaN($r.alignDeg) -and ![double]::IsNaN($zeroAlignDeg)) {
        $align = $r.alignDeg - $zeroAlignDeg
    } elseif ($useAngles -and ![double]::IsNaN($r.roll) -and ![double]::IsNaN($r.pitch)) {
...
$rightZoneStats = Side-ZoneStats $analysis "R"
$leftZoneStats = Side-ZoneStats $analysis "L"
...
$rotationZone = Rotation-Zone $avgRot
...
$hasAlternatingRotation = $rightRot -ge 12 -and $leftRot -ge 12 -and $symmetry -le 28
$hasUsefulEnergy = $avgImpulse -ge 0.25
$hasEnoughTime = $duration -ge 12
$sessionType = if (-not $hasEnoughTime -or ($avgRot -lt 6 -and $avgImpulse -lt 0.25)) {
    "SIN_NADO"
} elseif ($hasAlternatingRotation -and $hasUsefulEnergy -and $peaks.Count -ge 4) {
    "NADO_PROBABLE"
} elseif (($rightRot -ge 12 -or $leftRot -ge 12) -and $peaks.Count -ge 2) {
    "PRUEBA_TECNICA"
} else {
    "NO_CLASIFICADO"
}

if ($sessionType -ne "NADO_PROBABLE") {
    $rhythmLabel = "sin nado confirmado"
}
...
$speedRows = @($rows | Where-Object { -not [double]::IsNaN($_.speed) })
$avgSpeed = if ($speedRows.Count -gt 0) { Avg $speedRows { param($x) $x.speed } } else { 0.0 }
$maxSpeed = if ($speedRows.Count -gt 0) { ($speedRows | ForEach-Object { $_.speed } | Measure-Object -Maximum).Maximum } else { 0.0 }
function Advance-Label([double]$avg, [double]$max, [double]$distance, [int]$gpsCount) {
    if ($gpsCount -le 2 -and $max -lt 0.25) { return "0 mesa/sin GPS" }
    if ($gpsCount -le 2) { return "sin GPS fiable" }
    if ($max -lt 0.25 -and $distance -lt 3) { return "sin desplazamiento" }
    if ($avg -lt 0.8) { return "casi detenido" }
    if ($avg -lt 1.8) { return "lento/oleaje" }
    if ($avg -lt 3.2) { return "ritmo base" }
    if ($avg -lt 5.0) { return "ritmo fuerte" }
    return "vehiculo/no nado"
}
$advanceLabel = Advance-Label $avgSpeed $maxSpeed $distanceM $gpsRows.Count
if ($advanceLabel -eq "vehiculo/no nado") {
    $sessionType = "VEHICULO_NO_NADO"
    $rhythmLabel = "sin nado confirmado"
}
...
function Window-Score($items) {
    ...
    $rotScore = if ($rot -lt 20) {
        45 + $rot
    } elseif ($rot -le 55) {
        95 - [Math]::Abs(42 - $rot) * 0.6
    } elseif ($rot -le 65) {
        82 - ($rot - 55) * 1.2
    } else {
        62 - [Math]::Min(25, ($rot - 65) * 2.0)
    }
...
$simpleRotation = switch ($rotationLabel) {
    ...
    "moderada" { "el giro corporal apareció, pero todavía conviene compararlo con tu base personal." }
    "funcional" { "el giro corporal estuvo en una zona funcional inicial para analizar técnica." }
    "alta contextual" { "el giro fue alto; puede ser normal con respiración o nadadores entrenados si la línea se mantiene estable." }
    "muy alta" { "el giro fue muy alto; revisar si hubo respiración exagerada, pérdida de línea o mala colocación del sensor." }
...
$simpleImpulse = switch ($impulseLabel) {
    "suave" { "movimiento suave; puede ser nado tranquilo, mesa o poca energía relativa." }
    "normal" { "movimiento controlado y parejo." }
    "alto" { "hubo picos fuertes; mirar golpes, oleaje, contacto del dispositivo o movimiento brusco." }
}

$changeText = if (($impEnd - $impStart) -gt 0.4 -and ($alignEnd - $alignStart) -gt 2.0) {
    "al final subió el movimiento y cambió la alineación; posible fatiga, oleaje o técnica menos estable."
} elseif (($impEnd - $impStart) -gt 0.4) {
    "al final subió el movimiento; puede haber más intensidad, oleaje o contacto del dispositivo."
...
$sessionText = switch ($sessionType) {
    "NADO_PROBABLE" { "sesión compatible con nado: se detectó alternancia, energía y ciclos suficientes." }
    "PRUEBA_TECNICA" { "prueba técnica: hubo movimiento útil, pero no alcanza para llamarlo nado continuo." }
    "SIN_NADO" { "sin nado confirmado: prueba quieta, muy corta o con energía insuficiente." }
    "VEHICULO_NO_NADO" { "velocidad GPS incompatible con nado; usar esta sesión como prueba de GPS/traslado, no como sesión de atleta." }
    default { "datos no clasificados: revisar contexto de prueba antes de sacar conclusiones." }
}
$falsePositiveWarning = if ($sessionType -ne "NADO_PROBABLE") { "revisar_antes_de_contar_brazadas" } else { "no" }
$strokeTitle = if ($sessionType -eq "NADO_PROBABLE") { "Brazadas/ciclos detectados" } else { "Movimientos detectados" }
...
$metricRows = @(
    ...
    "rot_der_peak_avg_deg,$([Math]::Round($rightRot,2))",
    "rot_izq_peak_avg_deg,$([Math]::Round($leftRot,2))",
    "rot_der_max_deg,$([Math]::Round($rightZoneStats.maxDeg,2))",
    "rot_izq_max_deg,$([Math]::Round($leftZoneStats.maxDeg,2))",
    "rot_der_max_util_35_65_deg,$([Math]::Round($rightZoneStats.maxUsefulDeg,2))",
    "rot_izq_max_util_35_65_deg,$([Math]::Round($leftZoneStats.maxUsefulDeg,2))",
    "rot_der_max_funcional_35_55_deg,$([Math]::Round($rightZoneStats.maxFunctionalDeg,2))",
    "rot_izq_max_funcional_35_55_deg,$([Math]::Round($leftZoneStats.maxFunctionalDeg,2))",
    "rot_der_max_competitivo_55_65_deg,$([Math]::Round($rightZoneStats.maxCompetitiveDeg,2))",
    "rot_izq_max_competitivo_55_65_deg,$([Math]::Round($leftZoneStats.maxCompetitiveDeg,2))",
    "rot_der_general_avg_deg,$([Math]::Round($rightZoneStats.generalAvg,2))",
    "rot_izq_general_avg_deg,$([Math]::Round($leftZoneStats.generalAvg,2))",
    "rot_der_zone_35_65_s,$([Math]::Round($rightZoneStats.zoneSec,2))",
    "rot_izq_zone_35_65_s,$([Math]::Round($leftZoneStats.zoneSec,2))",
    "rot_der_zone_35_55_s,$([Math]::Round($rightZoneStats.functionalSec,2))",
    "rot_izq_zone_35_55_s,$([Math]::Round($leftZoneStats.functionalSec,2))",
    "rot_der_zone_55_65_s,$([Math]::Round($rightZoneStats.competitiveSec,2))",
    "rot_izq_zone_55_65_s,$([Math]::Round($leftZoneStats.competitiveSec,2))",
    "rot_der_max_streak_35_65_s,$([Math]::Round($rightZoneStats.maxStreakSec,2))",
    "rot_izq_max_streak_35_65_s,$([Math]::Round($leftZoneStats.maxStreakSec,2))",
    "rot_prom_deg,$([Math]::Round($avgRot,2))",
    "rotation_zone,$(Csv-Escape $rotationZone)",
    ...
    "movimiento_mag_prom,$([Math]::Round($avgImpulse,2))",
    "brazadas_estimadas,$($peaks.Count)",
    "session_type,$(Csv-Escape $sessionType)",
    "false_positive_warning,$(Csv-Escape $falsePositiveWarning)",
    ...
    "velocidad_prom_kmh,$([Math]::Round($avgSpeed,2))",
    "velocidad_max_kmh,$([Math]::Round($maxSpeed,2))",
    "avance_estado,$(Csv-Escape $advanceLabel)",
...
function Svg-Trend($data, [int]$width = 900, [int]$height = 260) {
    ...
    $series = @(
        ...
        @{ Name="movimiento"; Color="#5be98d"; Values=($data | ForEach-Object { $_.impulse * 8 }) }
    )
    $all = @()
    ...
    [void]$sb.Append("<text x='70' y='24' fill='#23d7ef' font-size='13'>rotación</text><text x='160' y='24' fill='#ffd166' font-size='13'>alineación</text><text x='260' y='24' fill='#5be98d' font-size='13'>movimiento x8</text>")
...
function Svg-Rotation($right, $left, [int]$width = 900, [int]$height = 270) {
    ...
    $leftGood = ArcPath $leftCx $cy ($r + 6) 35 55 "L"
    $rightGood = ArcPath $rightCx $cy ($r + 6) 35 55 "R"
    $leftOver = ArcPath $leftCx $cy ($r + 12) 55 65 "L"
    $rightOver = ArcPath $rightCx $cy ($r + 12) 55 65 "R"
...
<text x='$($leftCx-126)' y='$($cy-82)' fill='#5be98d' font-size='13'>35-55° funcional</text>
<text x='$($rightCx-8)' y='$($cy-82)' fill='#5be98d' font-size='13'>55-65° alto contextual</text>
...
function Svg-RotationSide([string]$label, [double]$degrees, [object]$stats, [string]$side, [string]$color, [int]$width = 430, [int]$height = 292) {
    function SvgNum([double]$v) { return $v.ToString("0.0", [Globalization.CultureInfo]::InvariantCulture) }
    $cx = $width / 2
    $cy = 132
    $lineY = 144
    $arm = 132
    $body = 86
    $d = Clamp $degrees 0 75
    $rad = $d * [Math]::PI / 180.0
    $dir = if ($side -eq "L") { -1.0 } else { 1.0 }
    $activeX = $cx + $dir * [Math]::Cos($rad) * $arm
    $activeY = $lineY - [Math]::Sin($rad) * $arm
    $passiveX = $cx - $dir * ($body * 0.82)
    $passiveY = $lineY + 11
    $hipX = $cx - $dir * 26
    $hipY = $lineY + 58
    $handX = $activeX + $dir * 22
    $handY = $activeY - 8
    $zone = if ($degrees -lt 20) { "baja" } elseif ($degrees -lt 35) { "moderada" } elseif ($degrees -le 55) { "funcional" } elseif ($degrees -le 65) { "alta contextual" } else { "muy alta" }
    $guide55Y = $lineY - [Math]::Sin(55 * [Math]::PI / 180.0) * 82
    $guide35Y = $lineY - [Math]::Sin(35 * [Math]::PI / 180.0) * 82
    $guide65Y = $lineY - [Math]::Sin(65 * [Math]::PI / 180.0) * 82
    $titleX = if ($side -eq "L") { 22 } else { $width - 190 }
    $smallX = if ($side -eq "L") { 22 } else { $width - 190 }
    $zoneText = ("zona 35-65: {0:N1}s" -f $stats.zoneSec)
    $streakText = ("racha max: {0:N1}s" -f $stats.maxStreakSec)
    $maxText = ("max útil {0:N1}° | max obs {1:N1}° | prom gen {2:N1}°" -f $stats.maxUsefulDeg, $stats.maxDeg, $stats.generalAvg)
    $competitiveText = ("competitivo 55-65: {0:N1}s | max comp {1:N1}°" -f $stats.competitiveSec, $stats.maxCompetitiveDeg)
    return @"
<svg viewBox='0 0 $width $height' role='img' aria-label='Rotación $label'>
<rect width='100%' height='100%' rx='10' fill='#071822'/>
<line x1='34' y1='$lineY' x2='$($width-34)' y2='$lineY' stroke='#285665' stroke-width='3'/>
<line x1='$cx' y1='50' x2='$cx' y2='$($height-42)' stroke='#214b58' stroke-width='2'/>
<line x1='58' y1='$(SvgNum $guide35Y)' x2='$($width-58)' y2='$(SvgNum $guide35Y)' stroke='#5be98d' stroke-width='2' stroke-dasharray='8 8' opacity='.55'/>
<line x1='58' y1='$(SvgNum $guide55Y)' x2='$($width-58)' y2='$(SvgNum $guide55Y)' stroke='#5be98d' stroke-width='2' stroke-dasharray='8 8' opacity='.75'/>
<line x1='58' y1='$(SvgNum $guide65Y)' x2='$($width-58)' y2='$(SvgNum $guide65Y)' stroke='#ff6a00' stroke-width='2' stroke-dasharray='8 8' opacity='.70'/>

<path d='M $(SvgNum $passiveX) $(SvgNum $passiveY) Q $(SvgNum ($cx - $dir * 70)) $(SvgNum ($lineY + 34)) $(SvgNum $hipX) $(SvgNum $hipY)' fill='none' stroke='#cfe5eb' stroke-width='7' stroke-linecap='round' opacity='.55'/>
<line x1='$(SvgNum $passiveX)' y1='$(SvgNum $passiveY)' x2='$cx' y2='$lineY' stroke='#cfe5eb' stroke-width='8' stroke-linecap='round' opacity='.45'/>
<line x1='$cx' y1='$lineY' x2='$(SvgNum $activeX)' y2='$(SvgNum $activeY)' stroke='$color' stroke-width='10' stroke-linecap='round'/>
<line x1='$(SvgNum $activeX)' y1='$(SvgNum $activeY)' x2='$(SvgNum $handX)' y2='$(SvgNum $handY)' stroke='$color' stroke-width='7' stroke-linecap='round'/>
<circle cx='$cx' cy='$($lineY-4)' r='24' fill='#eef8fb' opacity='.95'/>
<line x1='$(SvgNum ($cx-23))' y1='$lineY' x2='$(SvgNum ($cx+23))' y2='$lineY' stroke='#071822' stroke-width='4' opacity='.8'/>
<circle cx='$(SvgNum $activeX)' cy='$(SvgNum $activeY)' r='10' fill='$color'/>

<text x='$titleX' y='30' fill='$color' font-size='18' font-weight='800'>$label $([Math]::Round($degrees,1))° prom útil</text>
<text x='$smallX' y='54' fill='#cfe5eb' font-size='13'>$zone</text>
<text x='34' y='$($height-74)' fill='#cfe5eb' font-size='13'>$maxText</text>
<text x='34' y='$($height-52)' fill='#5be98d' font-size='13'>$zoneText | $streakText</text>
<text x='34' y='$($height-30)' fill='#ffb067' font-size='13'>$competitiveText</text>
<text x='34' y='$($height-12)' fill='#9bb5bf' font-size='12'>prom útil = promedio de picos; útil 35-65°; competitivo 55-65°</text>
</svg>
"@
}
...
function Svg-Impulse($impulse, [int]$width = 900, [int]$height = 150) {
    ...
    return @"
<svg viewBox='0 0 $width $height' role='img' aria-label='Movimiento observado'>
<rect width='100%' height='100%' rx='10' fill='#071822'/>
...
<text x='105' y='130' fill='#cfe5eb' font-size='16' font-weight='800'>movimiento $label | MAG $([Math]::Round($impulse,2)) promedio</text>
</svg>
"@
}

$trendSvg = Svg-Trend $analysis
$leftRotationSvg = Svg-RotationSide "IZQ" $leftRot $leftZoneStats "L" "#ff6a00"
$rightRotationSvg = Svg-RotationSide "DER" $rightRot $rightZoneStats "R" "#23d7ef"
...
Tipo de sesión: $sessionType | $sessionText

Rotación: $rotationLabel. $simpleRotation
Alineación: $alignmentLabel. $simpleAlignment
Movimiento/picos: $impulseLabel. $simpleImpulse
Ritmo: $rhythmLabel | ${strokeTitle}: $($peaks.Count)
GPS/avance: $advanceLabel | $($gpsRows.Count) puntos válidos | Distancia aproximada: $([Math]::Round($distanceM,1)) m | Velocidad prom $([Math]::Round($avgSpeed,1)) km/h | max $([Math]::Round($maxSpeed,1)) km/h
...
<section class="grid">
  ...
  <div class="card"><h2>Tipo de sesión</h2><div class="big">$sessionType</div><div class="muted">$(Html $sessionText)</div></div>
  <div class="card"><h2>GPS</h2><div class="big">$([Math]::Round($distanceM,1)) m</div><div class="muted">$($gpsRows.Count) puntos válidos</div></div>
  <div class="card"><h2>Avance GPS</h2><div class="big">$advanceLabel</div><div class="muted">prom $([Math]::Round($avgSpeed,1)) | max $([Math]::Round($maxSpeed,1)) km/h</div></div>
  ...
  <div class="card"><h2>Movimiento</h2><div class="big">$impulseLabel</div><div class="muted">MAG $([Math]::Round($avgImpulse,2)) promedio</div></div>
  <div class="card"><h2>Ritmo</h2><div class="big">$rhythmLabel</div><div class="muted">$($peaks.Count) $(Html $strokeTitle)</div></div>
  <div class="card span"><h2>Resumen para entender rápido</h2><div class="kid">
    ...
    <p><b>Avance GPS:</b> $(Html $advanceLabel), velocidad promedio $([Math]::Round($avgSpeed,1)) km/h y máxima $([Math]::Round($maxSpeed,1)) km/h.</p>
    <p><b>Movimiento/picos:</b> $(Html $simpleImpulse)</p>
    <p><b>Mejor tramo:</b> de $(Fmt-Seconds $bestStart) a $(Fmt-Seconds $bestEnd). Fue el tramo con mejor mezcla entre giro, línea corporal y movimiento estable.</p>
    <p><b>Cambio durante la sesión:</b> $(Html $changeText)</p>
  </div></div>
  <div class="card dark span"><h2>Tendencia de la sesión</h2>$trendSvg</div>
  <div class="card dark"><h2>Rotación izquierda</h2>$leftRotationSvg</div>
  <div class="card dark"><h2>Rotación derecha</h2>$rightRotationSvg</div>
  <div class="card dark span"><h2>Alineación corporal</h2>$alignmentSvg</div>
  <div class="card dark span"><h2>Movimiento MAG / picos</h2>$impulseSvg</div>
  <div class="card span"><h2>Qué mirar en la próxima prueba</h2>
    ...
    <p>Comparar si el lado dominante se reduce, si la alineación se mantiene estable, si el avance GPS es realista y si el movimiento aumenta sin perder línea corporal.</p>
  </div>
</section>
